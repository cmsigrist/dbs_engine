...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    val sql =
      """
      select
        col0, col5, col7, col10, count(*), min(col7)
      from lineitem_small
      where
        col0 > 2
      group by
        col0, col5, col7, col10
      order by
        col5,
        col10 desc
      """
...
"""
      ...
      from
        data, lineitem_small
      where
        data.col0 = lineitem_small.col0 and data.col0 = lineitem_small.col3
        """
*/
...
"""
*/
    val prep = SqlPrepare(Factories.VOLCANO_INSTANCE, "rowStore") //rleStore
