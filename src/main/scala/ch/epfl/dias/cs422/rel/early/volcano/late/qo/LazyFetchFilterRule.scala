...
class LazyFetchFilterRule protected (config: RelRule.Config)
  ...
  override def onMatchHelper(call: RelOptRuleCall): RelNode = {
    ...
    val lateColumnScan: LateColumnScan = call.rel(3)

    val fetch = new LogicalFetch(node, lateColumnScan.getRowType, lateColumnScan.getColumn, None)
    LogicalFilter.create(fetch, RexUtil.shift(filter.getCondition, 1)) // or +1
  }
}
